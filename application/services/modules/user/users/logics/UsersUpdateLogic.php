...
class UsersUpdateLogic
{
    ...
    public function _rules($data)
    {
        ...
        $this->ci->form_validation->set_rules('id', 'User ID', 'trim|required|integer');
        $this->ci->form_validation->set_rules('file_no', 'File No', 'trim|required|min_length[2]|max_length[15]');
        $this->ci->form_validation->set_rules('name', 'Full Name', 'trim|required|min_length[3]|max_length[255]');
