...
class MY_Model extends CI_Model
{
	...
	public function __construct()
	{
        ...
        */
		...
		// $this->after_update[] = 'returnFormatedData';
		// $this->after_create[] = 'returnFormatedData';
	}
		...
		if ($multi === FALSE) {
			foreach ($data_as_array as $field => $value) {
				if (in_array($field, $this->fillable)) {
					if ($value == '' || empty($value)) {
						$new_data[$field] = $value != 0 || $value != '0' ? null : $value;
					} else {
						$new_data[$field] = xssClean($value);
					}
				} else {
			...
			foreach ($data_as_array as $key => $row) {
				foreach ($row as $field => $value) {
					if (in_array($field, $this->fillable)) {
						if ($value == '' || empty($value)) {
							$new_data[$key][$field] = $value != 0 || $value != '0' ? null : $value;
						} else {
							$new_data[$key][$field] = xssClean($value);
						}
					} else {
						// show_error('MY_Model: Unknown column ' . $field . ' in table: ' . $this->table);
					}
		...
		if ($this->form_validation->run()) {
			...
			foreach ($rules as $rule) {
				if (in_array($rule['field'], $this->fillable)) {
					$this->validated[$rule['field']] = $this->input->post($rule['field'], TRUE);
				}
			}
	...
	private function _parse_model_dir($foreign_model)
	{
		...
		if ($full_model) {
			...
			$data['model_dir'] = str_replace($data['foreign_model'], (string) null, implode('/', $full_model));
		}
