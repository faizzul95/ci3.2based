...
async function deleteApi(id, url, reloadFunction = null) {
	if (id != '') {
		url = $('meta[name="base_url"]').attr('content') + url + '/' + id;
		try {
			return axios({
					method: 'DELETE',
					headers: {
...
function loadingBtn(id, display = false, text = "<i class='ti ti-device-floppy ti-xs mb-1'></i> Save") {
	if (display) {
...
function printDiv(idToPrint, printBtnID = 'printBtn', printBtnText = "<i class='ti ti-device-floppy ti-xs mb-1'></i> Save", pageTitlePrint = 'Print') {
	$("#" + idToPrint).printThis({
...
function skeletonTableCard(hasFilter = null, buttonRefresh = true) {
